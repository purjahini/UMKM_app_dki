...
class AddProdukActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(com.svtech.mandiri.R.layout.activity_produk_custome)
...
//                            KategoriList)
                    ...
                    spnKategoriProduk.setSelection(
                        See.getIndex(
                            arrayList, spnKategoriProduk,
                            kategori.toString()
                        )
                    )
...
//            pickImage(this, imgFoto,"Produk")
        ...
        minimal_pembelian = intent.getIntExtra("minimal_pembelian", 0)
...
//            else hideDiskonInput()
    ...
    private fun addProdukId() {
        ...
        val minimal_pembelian = textInputMinimalPembelian.editText?.text.toString().trim()
        ...
        AndroidNetworking.upload(MyConstant.Urlprodukcreate)
            ...
            .addMultipartParameter(
                "minimal_pembelian",
                if (minimal_pembelian.isNullOrEmpty()) 0.toString() else minimal_pembelian
            )
...
//        val minimal_pembelian =  textInputMinimalPembelian.editText?.text.toString().trim()
        ...
        AndroidNetworking.upload(MyConstant.Urlprodukupdateid)
            .addHeaders(MyConstant.AUTHORIZATION, MyConstant.BEARER + token)
            .addMultipartParameter(MyConstant.ID, produkId.toString())
            .addMultipartParameter(
                MyConstant.DISKON,
            ...
            .addMultipartParameter(MyConstant.NAMA, textInputNamaProduk.editText?.text.toString())
            .addMultipartParameter(MyConstant.SATUAN,  spnSatuan.selectedItem as String)
            .addMultipartParameter(MyConstant.STOK, textInputStok.editText?.text.toString())
            ...
            .getAsJSONObject(object : JSONObjectRequestListener{
                override fun onResponse(response: JSONObject?) {
                    val respon = response.toString()
                    See.log("response update produk $respon")
                    val json = JSONObject(respon)
                    val apiMessage = json.getString(MyConstant.API_MESSAGE)
                    val apiStatus = json.getInt(MyConstant.API_STATUS)
                    if (apiStatus.equals(1)){
                ...
                override fun onError(anError: ANError?) {
                    ...
                    See.log("onError update errorCode : ${anError?.errorCode}")
                    See.log("onError update errorBody : ${anError?.errorBody}")
                    See.log("onError update errorDetail : ${anError?.errorDetail}")
